...
// IMPORT DB PIPELINE
import { createProjectInDB, updateProjectInDB, updateProjectScope } from "@/lib/data/project-actions";
...
export function ProjectWizard({ onClose, onCreate, initialData, projectId }: ProjectWizardProps) {
  ...
  const [data, setData] = useState<ProjectData>({
    ...
    intent: initialData?.intent || 'internal',
    description: initialData?.description || '',
    successType: initialData?.successType || 'undefined',
    ...
    targetDeadline: initialData?.targetDeadline || '',
    structure: initialData?.structure || 'linear',
  ...
  const handleSaveToDatabase = async () => {
    ...
    try {
      ...
      // 1. Simpan/Update Proyek ke tabel 'projects'
      let savedProject;
      if (projectId) {
          savedProject = await updateProjectInDB(projectId, {
              name: data.name,
      ...
      // 2. Jika proyek berhasil dibuat/diupdate, Simpan Scope of Work-nya
      if (savedProject && savedProject.id) {
         ...
         if (inScopeArr.length > 0 || outOfScopeArr.length > 0) {
             await updateProjectScope(savedProject.id, inScopeArr, outOfScopeArr);
         }
      }

      toast.success(projectId ? "Perubahan proyek berhasil disimpan!" : "Proyek baru berhasil dibuat!");
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1, height: "auto" }} transition={{ duration: 0.3 }} className="flex w-full max-w-[900px] max-h-[85vh] overflow-hidden rounded-[24px] bg-background shadow-2xl">
        <div className="hidden w-64 border-r border-border bg-background px-6 py-7 md:flex md:flex-col md:gap-7">
          <p className="text-sm font-semibold text-foreground">{projectId ? "Edit Detail Proyek" : "Setup Proyek Baru"}</p>
                ...
                <div className="flex gap-3">
                    {step === 5 ? (
                        <>
                            ...
                            <Button onClick={handleSaveToDatabase} disabled={isSubmitting}>{isSubmitting ? <Spinner className="animate-spin mr-2" /> : null} {projectId ? "Simpan Perubahan" : "Simpan Proyek"}</Button>
                        </>
                    ) : (
