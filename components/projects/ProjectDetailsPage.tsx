...
import { updateProjectScope, deleteProjectInDB } from "@/lib/data/project-actions"
...
export function ProjectDetailsPage({ projectId, dbData }: { projectId: string, dbData?: any }) {
  ...
  const mappedFiles = rawFiles.map((f: any) => ({
    ...
    type: f.type || f.file_type || "file",
    size: `${f.size_mb || f.file_size_mb || 0} MB`,
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      toast.success("Proyek berhasil dihapus selamanya.");
  ...
  return (
    <div className="flex flex-1 flex-col min-w-0 m-2 border border-border rounded-lg bg-background shadow-sm">
            ...
            <div className={`mt-0 grid grid-cols-1 gap-10 ${showMeta ? "lg:grid-cols-[minmax(0,2fr)_minmax(0,320px)]" : "lg:grid-cols-[minmax(0,1fr)_minmax(0,0px)]"}`}>
              <div className="space-y-6 pt-6 pb-12">
                <ProjectHeader 
                  ...
                  onEditProject={() => setIsWizardOpen(true)} 
      ...
      {isWizardOpen && (
        <ProjectWizard 
          ...
          initialData={{
            ...
            intent: project.targets.intentLabel.toLowerCase() as any,
            targetProduksi: project.targets.targetProduksi,
            targetRevenue: project.targets.targetRevenue,
            pic: dbData?.pic_name,
            client: project.source?.client,
            inScope: project.scope.inScope.join('\n'),
            outOfScope: project.scope.outOfScope.join('\n'),
            targetDeadline: dbData?.end_date ? new Date(dbData.end_date).toISOString().split('T')[0] : undefined,
            structure: dbData?.structure_type?.toLowerCase() as any,
            city: dbData?.location?.split(',')[0]?.trim(),
            province: dbData?.location?.split(',')[1]?.trim(),
          }}
        />
      )}
      ...
      <DeleteConfirmModal 
        ...
        description="Apakah Bapak yakin ingin menghapus seluruh data proyek ini? Tindakan ini akan menghapus semua tugas, catatan, dan file di dalamnya secara permanen."
