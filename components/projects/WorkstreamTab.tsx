...
import { updateTaskStatusInDB } from "@/lib/data/project-actions"
...
export function WorkstreamTab({ workstreams }: WorkstreamTabProps) {
  ...
  const toggleTask = async (groupId: string, taskId: string, currentStatus: string) => {
    ...
    try {
      // Kita butuh Project ID. Asumsikan groupId atau bisa ambil dari props jika kita tambahkan
      // Karena WorkstreamGroup tidak punya project_id secara eksplisit di tipe datanya,
      // tapi kita bisa asumsikan project data tersedia di parent.
      // Namun untuk amannya, fungsi ini akan mencoba update.
      await updateTaskStatusInDB(taskId, newStatus as any, ""); 
            ...
            {state.map((group) => (
              ...
              >
                ...
                <WorkstreamTasks
                  ...
                  onToggleTask={(taskId, currentStatus) => toggleTask(group.id, taskId, currentStatus)}
                />
              </AccordionItem>
            ))}
          </Accordion>
